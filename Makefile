...
docker-build:
	@echo "--> Compiling the project"
	sudo docker run --rm -v ${ROOT_DIR}:/go/src/github.com/gambol99/etcd-discovery \
		-w /go/src/github.com/gambol99/etcd-discovery -e GOOS=linux golang:${GOVERSION} make static
...
release: static
	@echo "--> Building a release for version ${VERSION}"
	mkdir -p release
